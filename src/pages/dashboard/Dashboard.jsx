...
const Dashboard = () => {
    ...
    return (
        <PageWrapper>
            <main className="flex-grow max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-32 w-full">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                    <div className="flex items-center gap-4">
                        ...
                        {healthStatus && role === 'admin' && (
                            <div className={`px-3 py-1 mt-1 rounded-full text-xs font-bold uppercase tracking-wider border flex items-center gap-1 ${healthStatus === 'healthy' ? 'bg-green-100 text-green-700 border-green-200' : 'bg-red-100 text-red-700 border-red-200'}`}>
                                <ShieldCheck size={14} />
                                {healthStatus}
                            </div>
                        )}
                    ...
                    <div className="flex gap-3">
                        {role === 'manager' && (
                            <button onClick={() => setShowAlertModal(true)} className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-indigo-600 to-violet-600 text-white rounded-xl hover:from-indigo-500 hover:to-violet-500 transition-all shadow-sm shadow-indigo-200 font-medium whitespace-nowrap">
                                <Bell size={18} /> Send Alert
                            </button>
                        )}
                        {role === 'admin' && (
                            <div className="flex items-center gap-3">
                                <button onClick={() => setShowAdminAlertModal(true)} className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-violet-600 to-fuchsia-600 text-white rounded-xl hover:from-violet-500 hover:to-fuchsia-500 transition-all shadow-sm shadow-violet-200 font-medium whitespace-nowrap">
                                    <Bell size={18} /> Broadcast Alert
                                </button>
                                <button onClick={handleTriggerEscalations} className="flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition-colors shadow-sm shadow-indigo-200 font-medium whitespace-nowrap">
                ...
                {/* Tab Navigation */}
                <div className="flex px-1 space-x-2 bg-slate-100/50 dark:bg-zinc-800/50 p-1.5 rounded-2xl mb-8 overflow-x-auto custom-scrollbar w-fit">
                    {tabs.map((tab) => (
                        <button
                            key={tab.id}
                            ...
                            className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-semibold text-sm transition-all whitespace-nowrap ${activeTab === tab.id
                                ? 'bg-[#c7d2fe] dark:bg-zinc-700 text-indigo-700 dark:text-indigo-300 shadow-sm border border-slate-200/50 dark:border-zinc-600/50'
            ...
            {selectedComplaint && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
                    ...
                    <div className="bg-[#c7d2fe] dark:bg-zinc-900 rounded-3xl shadow-2xl w-full max-w-lg relative z-10 overflow-hidden transform transition-all animate-in fade-in zoom-in-95 duration-200">
                        <div className="px-6 py-5 border-b border-slate-100 dark:border-zinc-800 flex justify-between items-center bg-slate-50/50 dark:bg-zinc-800/50">
                            <div className="flex items-center gap-3">
                                ...
                                <span className="font-mono text-xs text-slate-500 dark:text-zinc-400 bg-[#c7d2fe] dark:bg-zinc-800 px-2 py-1 rounded-md border border-slate-200 dark:border-zinc-700 shadow-sm">#{selectedComplaint.id}</span>
                            </div>
                            <div className="flex items-center gap-2">
                                ...
                                <button onClick={() => setSelectedComplaint(null)} className="text-slate-400 hover:text-slate-700 bg-[#c7d2fe] shadow-sm p-1.5 rounded-full border border-slate-200" title="Close">
                                    <X size={18} />
                                </button>
                            </div>
                        </div>
                                ...
                                <div className="flex flex-wrap gap-3">
                                    {['pending', 'in_progress'].map((status) => (
                                        <button
                                            ...
                                            className={`py-2 px-4 rounded-xl text-xs font-bold uppercase tracking-wide border transition-all ${selectedComplaint.status === status
                                                ...
                                                : 'bg-[#c7d2fe] dark:bg-zinc-800 text-slate-600 dark:text-zinc-300 border-slate-200 dark:border-zinc-700 hover:bg-slate-50 dark:hover:bg-zinc-700'
                                                }`}
                                        >
                                            {status.replace('_', ' ')}
                                        </button>
                                    ...
                                    <button
                                        ...
                                        className={`py-2 px-4 rounded-xl text-xs font-bold uppercase tracking-wide border transition-all ${selectedComplaint.status === 'resolved'
                                            ...
                                            : 'bg-[#c7d2fe] text-emerald-700 border-emerald-200 hover:bg-emerald-50'
                                            }`}
                                    >
                                        Resolved (Review)
                                    </button>
                                        ...
                                        <>
                                            <button
                                                ...
                                                className={`py-2 px-4 rounded-xl text-xs font-bold uppercase tracking-wide border transition-all ${selectedComplaint.status === 'closed'
                                                    ...
                                                    : 'bg-[#c7d2fe] text-slate-700 border-slate-300 hover:bg-slate-100'
                                                    }`}
                                            >
                                                Close Ticket
                                            </button>
                                            <button
                                                ...
                                                className={`py-2 px-4 rounded-xl text-xs font-bold uppercase tracking-wide border transition-all ${selectedComplaint.status === 'rejected'
                                                    ...
                                                    : 'bg-[#c7d2fe] text-red-600 border-red-200 hover:bg-red-50'
                                                    }`}
                                            >
                                                Reject Task
                                            </button>
